...
from sub_ocr.utils import flatten_iter, pairwise_tuples, rect_corners
from utilities.visualize import visualize_data

DATASET_DIR = Path(r"C:\Users\Victor\Documents\Datasets\Subtitle_OCR")
        ...
        if self.model_type == "rec":
            for image_path, label_path in zip(image_files, image_labels):
                for label in label_path.read_text("utf-8-sig").splitlines():
                    ...
                    bbox, text = pairwise_tuples(map(float, label[:8])), label[8:][0]
...
def load_data(lang: str, model_type: str, data_type: str) -> list:
    if lang == "en":
        if model_type == "det":
        ...
        if model_type == "det":
            return merge_data_sources(ICDAR2015Data(data_type))
        elif model_type == "rec":
            return merge_data_sources(ICDAR2015Data(data_type, model_type))
