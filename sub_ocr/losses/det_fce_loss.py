...
class FCELoss(nn.Module):
    ...
    def forward_single(self, pred, gt):
        ...
        tr_train_mask = (train_mask * tr_mask).type("bool")
        ...
        if tr_train_mask.sum().item() > 0:
            loss_tcl_pos = F.cross_entropy(
                ...
                tcl_mask.masked_select(tr_train_mask).type("int64"),
            )
            loss_tcl_neg = F.cross_entropy(
                tcl_pred.masked_select(tr_neg_mask2).reshape([-1, 2]),
                tcl_mask.masked_select(tr_neg_mask).type("int64"),
            )
        ...
        loss_reg_y = torch.tensor(0.0).type(torch.float32)
        if tr_train_mask.sum().item() > 0:
            weight = (tr_mask.masked_select(tr_train_mask.type("bool")).type(torch.float32)
                      + tcl_mask.masked_select(tr_train_mask.type("bool")).type(torch.float32)) / 2
    ...
    def ohem(self, predict, target, train_mask):
        pos = (target * train_mask).type("bool")
        neg = ((1 - target) * train_mask).type("bool")
        ...
        if n_pos.item() > 0:
            loss_pos = F.cross_entropy(
                ...
                target.masked_select(pos).type("int64"),
                reduction="sum",
            )
            loss_neg = F.cross_entropy(
                predict.masked_select(neg2).reshape([-1, 2]),
                target.masked_select(neg).type("int64"),
                reduction="none",
            )
            n_neg = min(int(neg.type(torch.float32).sum().item()), int(self.ohem_ratio * n_pos.type(torch.float32)))
        else:
            loss_pos = torch.tensor(0.0)
            loss_neg = F.cross_entropy(predict.masked_select(neg2).reshape([-1, 2]),
                                       target.masked_select(neg).type("int64"), reduction="none")
            n_neg = 100
