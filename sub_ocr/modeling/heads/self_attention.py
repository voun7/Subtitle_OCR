...
class WrapEncoderForFeature(nn.Module):
    def __init__(self, src_vocab_size, max_length, n_layer, n_head, d_key, d_value, d_model, d_inner_hid,
                 prepostprocess_dropout, attention_dropout, relu_dropout, preprocess_cmd, postprocess_cmd,
                 weight_sharing, bos_idx=0):
        super().__init__()
        ...
        self.encoder = Encoder(n_layer, n_head, d_key, d_value, d_model, d_inner_hid, prepostprocess_dropout,
...
class WrapEncoder(nn.Module):
    ...
    def __init__(self, src_vocab_size, max_length, n_layer, n_head, d_key, d_value, d_model, d_inner_hid,
        ...
        self.prepare_decoder = PrepareDecoder(src_vocab_size, d_model, max_length, prepostprocess_dropout,
                                              bos_idx=bos_idx)
        self.encoder = Encoder(n_layer, n_head, d_key, d_value, d_model, d_inner_hid, prepostprocess_dropout,
                               attention_dropout, relu_dropout, preprocess_cmd, postprocess_cmd)
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_layer, n_head, d_key, d_value, d_model, d_inner_hid, prepostprocess_dropout, attention_dropout,
        ...
        for i in range(n_layer):
            encoderLayer = EncoderLayer(n_head, d_key, d_value, d_model, d_inner_hid, prepostprocess_dropout,
                                        attention_dropout, relu_dropout, preprocess_cmd, postprocess_cmd)
    ...
    def __init__(self, process_cmd, d_model, dropout_rate):
        ...
        for cmd in self.process_cmd:
            if cmd == "a":  # add residual connection
                self.functors.add_module('add_res_connect_{}'.format(cur_a_len),
                                         LambdaXY(lambda x, y: x + y if y is not None else x))
                cur_a_len += 1
            elif cmd == "n":  # add layer normalization
                layerNorm = torch.nn.LayerNorm(normalized_shape=d_model, elementwise_affine=True, eps=1e-5)
                self.functors.add_module("layer_norm_%d" % cur_n_len, layerNorm)
                cur_n_len += 1

            elif cmd == "d":  # add dropout
                self.functors.add_module('add_drop_{}'.format(cur_d_len), Lambda(lambda x: F.dropout(x, p=dropout_rate)
...
class PrepareEncoder(nn.Module):
    def __init__(self, src_vocab_size, src_emb_dim, src_max_len, dropout_rate=0, bos_idx=0, word_emb_param_name=None,
...
class PrepareDecoder(nn.Module):
    def __init__(self, src_vocab_size, src_emb_dim, src_max_len, dropout_rate=0, bos_idx=0, word_emb_param_name=None,
        ...
        self.src_emb_dim = src_emb_dim
        """
        self.emb0 = Embedding(num_embeddings=src_vocab_size, embedding_dim=src_emb_dim)
        """
