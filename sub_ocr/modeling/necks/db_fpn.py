...
class DSConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride=1, groups=None, if_act=True, act="relu",
                 **kwargs):
        super().__init__()
        if groups == None:
            groups = in_channels
...
class LKPAN(nn.Module):
    def __init__(self, in_channels, out_channels, mode='large', **kwargs):
        ...
        if mode.lower() == 'lite':
            ...
            raise ValueError(f"mode can only be one of ['lite', 'large'], but received {mode}")
