...
class GridGenerator(nn.Module):
    ...
    def build_P_paddle(self, I_r_size):
        ...
        I_r_grid_x = (torch.arange(-I_r_width, I_r_width, 2, dtype=torch.float64) + 1.0
                      ) / torch.as_tensor(np.array([I_r_width]).astype(np.float64))

        I_r_grid_y = (torch.arange(-I_r_height, I_r_height, 2, dtype=torch.float64) + 1.0
    ...
    def build_inv_delta_C_paddle(self, C):
        ...
        delta_C = torch.cat(  # F+3 x F+3
            [
                torch.cat([torch.ones((F, 1), dtype=torch.float64), C, hat_C], dim=1),  # F x F+3
                torch.cat([torch.zeros((2, 3), dtype=torch.float64), C.permute(1, 0)], dim=1),  # 2 x F+3
                torch.cat([torch.zeros((1, 3), dtype=torch.float64),
                           torch.ones((1, F), dtype=torch.float64)], dim=1)  # 1 x F+3
            ], dim=0)
