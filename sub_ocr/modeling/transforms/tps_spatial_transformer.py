...
def grid_sample(input, grid, canvas=None):
    ...
    output = F.grid_sample(input, grid, align_corners=True) if torch.__version__ >= '1.3.0' else F.grid_sample(input,
                                                                                                               grid)
    if canvas is None:
        return output
    else:
...
def compute_partial_repr(input_points, control_points):
    ...
    pairwise_diff = torch.reshape(
        input_points, shape=[N, 1, 2]) - torch.reshape(control_points, shape=[1, M, 2])
