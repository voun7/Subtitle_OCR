...
class MakeBorderAndShrinkMap:
    def __init__(self, image_height: int, image_width: int, shrink_ratio: float = 0.4, thresh_min: float = 0.3,
                 thresh_max: float = 0.7) -> None:
    ...
    def get_maps(self, anns: list) -> tuple:
        anns = [ann for ann in anns if Polygon(ann['bbox']).buffer(0).is_valid]
        gt = np.zeros((self.image_height, self.image_width), dtype=np.float32)
        mask = np.ones((self.image_height, self.image_width), dtype=np.float32)
        thresh_map = np.zeros((self.image_height, self.image_width), dtype=np.float32)
        thresh_mask = np.zeros((self.image_height, self.image_width), dtype=np.float32)
...
def db_preprocess(image_path: str, image: np.ndarray, anns: list, image_height: int, image_width: int) -> dict:
    ...
    data = {
        ...
        "shrink_map": gt, "shrink_mask": mask, "threshold_map": thresh_map, "threshold_mask": thresh_mask,
    }
    return data
